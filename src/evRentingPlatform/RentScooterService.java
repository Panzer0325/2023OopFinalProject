...
import evRentingPlatformGUI.GoogleMapsGUI;
...
public class RentScooterService {
	...
	public Repairman getrpOperator() {
		return rpOperator;
	}
	/**
...
//            }        
	...
	public boolean moveToChargingStation(User user, ChargingStation chargingStation) {
		try {
			...
			if(user.getScooter().getPower() > distance) {
				...
				UserGUI.loadMap(chargingStation.getPosition().lat,chargingStation.getPosition().lng);
				GoogleMapsGUI.loadMap(chargingStation.getPosition().lat,chargingStation.getPosition().lng);
				return true;
			}
			return false;
		}catch(Exception e) {
	...
	public boolean chargeScooter(User user){
		try {
			...
			user.getRentEvent().addChargeTimes();
